...
const reducer = (state, { type, index }) => {
	...
	switch(type) {
		case INC:
			return {...state, direction: 1, count: isCyclic ? (count + 1 + total) % total : clamp(count + 1, start, end)}
		case DEC:
			return {...state, direction: -1, count: isCyclic ? (count - 1 + total) % total : clamp(count - 1, start, end)}
		case GOTO:
...
const useCounter = ({ count = 0, direction = 0, start = 0, end = 4, isCyclic = false }) => {
    const initialState = { count, direction, start, end, isCyclic }
    ...
    // const goto = useCallback(index => dispatch({ type: GOTO, index }), [dispatch]);
        ...
        const goto = useCallback(index => {
            ...
            return dispatch({ type: GOTO, index })
        }, [dispatch]);
